...
export default class HelpDesk {
   ...
   showRemoveForm(ticket) {
      ...
      this.removeForm.querySelector('.ok-button').addEventListener('click', (evt) => {
         ...
         xhr.open('POST', `${this.url}?deleteById&id=${ticket.id}`, true);
         xhr.send(ticket.id);
         ticket.remove();
   ...
   showEditForm(ticket) {
      ...
      this.editForm.querySelector('.ok-button').addEventListener('click', (evt) => {
         ...
         //
         const xhr = new XMLHttpRequest();
         xhr.open('POST', `${this.url}?method=updateById&id=${ticket.id}`, true);
         xhr.send(editTicket);
   ...
   createTicket(shortDesc, fullDesc, status, created) {
      ...
      ticket.querySelector('.ticket-status').addEventListener('click', (evt) => {
         console.log("🚀 ~ file: HelpDesk.js:127 ~ HelpDesk ~ ticket.querySelector ~ evt:", evt)

         const xhr = new XMLHttpRequest();
         xhr.open('POST', `${this.url}?method=createTicket=${ticket.id}`, true);
         xhr.send(ticket.id);
   ...
   addNewTicket() {
      ...
      const xhr = new XMLHttpRequest();
      xhr.open('POST', `${this.url}?method=createTicket`, true);
      xhr.send(newTicket);
   ...
   redrawAllTickets() {
      const xhr = new XMLHttpRequest();
      xhr.open('GET', `${this.url}?method=allTickets`, true);
